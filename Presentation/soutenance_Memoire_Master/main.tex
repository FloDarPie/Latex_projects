...
\begin{frame}{Structure de travail}
    \begin{blockSimple}{}
        \centering
...
\tikzstyle{arrow} = [thick,->,>=stealth, line width=1.5pt]
    ...
    \begin{blockSimple}{Vérification de binaire}
        $\centerdot$ Approche simple
        $\centerdot$ Approche systématique
        $\centerdot$ Approche automatique
    \end{blockSimple}
    \pause
...
reach <puts> such that @[rdi, 14] = "Good password!" then print ascii stream password
...
\begin{tikzpicture}[overlay , remember picture, node distance=0.5cm]
    ...
    \onslide<5->{
    \node[stop, fit={(0.3,0.1) (12.74,0.7)}, label={[labelshift]center:Stop}]{};}
    \onslide<4->{\node[main, fit={(0.3,1.35) (12.74,2.3)}, label={[labelshift]center:Stubs}]{};}
    \onslide<3->{\node[definition, fit={(0.3,2.9) (12.74,5.5)}, label={[labelshift]center:Initialisation de registres}]{};}
    \onslide<2->{\node[intro, fit={(0.3,6.1) (12.74,6.1)}, label={[labelshift, yshift=-0.5cm]center:Chargement des données}]{};}
