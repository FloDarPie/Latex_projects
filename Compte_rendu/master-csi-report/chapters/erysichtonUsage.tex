...
L'éxécution c'est réalisé sur une machine équipée d'un processeur \textit{Intel Xeon E5-2620v4} avec 32 Gio de mémoire. Le temps nécéssaire pour une analyse complète en x86\_64 avec \texttt{GCC} est de 4h07. Une analyse complète comprend la compilation de Hacl*, la génération des fichiers de tests, la compilation des-dits fichiers et l'analyse individuelle de chacun par Binsec. Nous avons ajouter un module pour synthétiser et générer des rapports d'analyse.\medbreak
...
Les données détaillées peuvent être consulté en annexe \ref{tab:resultats_finaux}. En l'état, l'analyse rapporte entre 139 et 168 fichiers dont l'analyse n'a pus se terminer. Il faudrait observer plus en détails ces fichiers pour connaître les causes de ces arrêts. \medbreak
...
Le détails des valeurs est rapporté en annexe \ref{table:detail_unknown}.\smallbreak

On peut voir avec cette figure \ref{fig:graphe_unknown} que les erreurs sont dues à :
\begin{enumerate}
  \item[\texttt{max depth}] Arrêt par limitations du nombre d'instruction à analyser, cela permet de réduire la profondeur des branchement conditionnels à explorer et limiter le risque de parcour infini.
  ...
  \item[\texttt{killed}] Consommation excessive des ressources, processus terminé. 
\end{enumerate}
...
Revenons sur les resultats présentés sur la figure \ref{fig:graphe_total} et ignorons les fonctions marquées \texttt{unknown}. Les fichiers non sécurisés sont les plus nombreux avec l'option \texttt{-O3} (27) et le moins avec l'option \texttt{-O0} (21). Cette expérimentation remet en évidence les travaux de \citeauthor{schneider2024breakingbadcompilersbreak} mais nous pouvons nous intéroger quant à la réalité des fuites présentés ici. Comme nous analysons toute la bibliothèque Hacl* certaines fonctions peuvent être non sécurisées mais cela n'impacte pas la sécurité globale car elles ne sont besoin d'être sécurisé
