...
\begin{itemize}
    \item[\cite{DPA_Attack}] Consommation d'énergie %- lecture des niveaux de consommation d'électricité pour déterminer les opérations executées
    \item[\cite{Branch_Attack}] Prédiction de branchement %- 
    \item[\cite{Thermal_Attack}] Variation de température %- permet la prédicton d'application executé
    \item[\cite{DRAM_Attack}] Accès à la mémoire DRAM %- exploite des lignes mémoires partagées entre différents CPU
