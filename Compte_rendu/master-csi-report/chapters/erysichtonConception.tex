...
Nous avons notre besoin, nous avons nos exigences, nous avons fait des tests pour comprendre le processus que l'on va devoir automatser. Ces idées peuvent être illustrées par la figure \ref{fig:compilation_simple} : depuis Hacl*, on extrait une fonction que l'on veut tester en un fichier de test en C, on identifie les paramètres secrets et on conçois le script pour Binsec, on compile notre fichier C à notre guise, on lance l'analyse Binsec.
