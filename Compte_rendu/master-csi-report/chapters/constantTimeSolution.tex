...
Ce deuxième chapitre montre les innovations nécessaires pour se protéger des attaques temporelles. On y découvre les bonne pratiques de programmation, les premiers outils automatique de vérification de code ainsi que les limitations auquel est confronté le développeur qui souhaite être résistant à ces attaques.
...
La programmation en temps constant, \textit{Constant-Time Programming}, est une pratique de programmation qui vise a résoudre exactement ce problème. Directement lié à la compléxité algorithmique, cette pratique modifie et adapte les algorithmes pour que toutes les opérations effectuées est un temps d'exécution identique.
...
\begin{CitationBox}{Accès mémoire}
  Un chargement depuis la mémoire d'une information est une source de variation. On a vus précedemment \cite{LLC_attack,DRAM_Attack} que l'usage d'un cache mémoire est un canal d'accès pour réaliser une attaque. En effet, l'utilisation d'un cache permet de distinguer les appels entre les données déjà mise en mémoire ou pas encore; de plus, les changement de valeurs dans celui-ci peuvent aussi être observé après exécution.
\end{CitationBox}
\vspace{0.1cm}
\begin{CitationBox}{Décalage et rotation}
  Ces opérations binaires sont ou ne sont pas en temps constant en fonction des CPU sur lequel le code est exécuté. Certain ont un "barrel shifter" qui permet d'effectuer directement les instructions correspondantes. Cela impacte directement les algorithmes dépendant de décalages logiques comme le chiffrement RC5.
\end{CitationBox}
\vspace{0.1cm}
...
Avec le compilateur \texttt{RISC-V rv64gc clang 15.0.0}, si on entre les options de compilation \texttt{-O0} ou \texttt{-O1} on peut observer différents résultats. Le plus notable ici est l'apparition de l'instruction \texttt{beqz}, qui est un branchement conditionnel. Les optimisations appelées par l'option \texttt{-O1} permettent d'identifier le tour de passe passe qui lui été proposé. Le code \ref{subcode:O0} suient les instructions précisées par le code source, le compilateur avec cette optimisation compile vite, à l'inverse du code \ref{subcode:O1} où les sauts succesifs entre les \texttt{beqz} permet une exécution plus rapide. Les options de compialtions sont rapportées en annexe \ref{tab:compile_option}\footnote{\url{https://gcc.gnu.org/}}.
